...
from kiro.config import (
    ...
    TOOL_RESULT_CACHE_KEY_PREFIX,
    _warn_timeout_configuration,
)
from kiro.auth import KiroAuthManager
from kiro.cache import ModelInfoCache
from kiro.model_resolver import ModelResolver
from kiro.prompt_cache import RedisPromptCache
from kiro.response_cache import RedisResponseCache
from kiro.tool_result_cache import RedisToolResultCache
...
async def lifespan(app: FastAPI):
    ...
    # Create exact response cache
    app.state.response_cache = RedisResponseCache(
        redis_url=RESPONSE_CACHE_REDIS_URL,
        ttl_seconds=RESPONSE_CACHE_TTL_SECONDS,
        key_prefix=RESPONSE_CACHE_KEY_PREFIX,
    )
    await app.state.response_cache.connect()

    app.state.prompt_cache = RedisPromptCache(
        redis_url=PROMPT_CACHE_REDIS_URL,
        key_prefix=PROMPT_CACHE_KEY_PREFIX,
    )
    await app.state.prompt_cache.connect()

    app.state.tool_result_cache = RedisToolResultCache(
        redis_url=RESPONSE_CACHE_REDIS_URL,
        ttl_seconds=TOOL_RESULT_CACHE_TTL_SECONDS,
        key_prefix=TOOL_RESULT_CACHE_KEY_PREFIX,
    )
    await app.state.tool_result_cache.connect()
    ...
    try:
        await app.state.response_cache.close()
        logger.info("Response cache client closed")
    except Exception as e:
        logger.warning(f"Error closing response cache client: {e}")

    try:
        await app.state.prompt_cache.close()
        logger.info("Prompt cache client closed")
    except Exception as e:
        logger.warning(f"Error closing prompt cache client: {e}")

    try:
        await app.state.tool_result_cache.close()
        logger.info("Tool result cache client closed")
    except Exception as e:
