...
class TestProcessToolsWithLongDescriptions:
    ...
    def test_long_description_truncated_by_default(self):
        """
        What it does: Verifies long descriptions are truncated by default.
        Purpose: Ensure tool docs do not get copied into the system prompt.
        """
        ...
        print("Checking truncated description...")
        assert len(processed) == 1
        assert processed[0].description.endswith("... [truncated by gateway]")
        assert len(processed[0].description) <= 10000

        print("Checking documentation is not mirrored into system prompt...")
        assert doc == ""

    def test_mixed_short_and_long_descriptions(self):
        """
        What it does: Verifies handling of mixed tools list.
        Purpose: Ensure short ones stay, long ones are truncated.
        """
        ...
        assert processed[1].description.endswith("... [truncated by gateway]")
        assert len(processed[1].description) <= 10000
        assert doc == ""
    ...
    def test_multiple_long_descriptions_all_moved(self):
        ...
        for tool in processed:
            assert tool.description.endswith("... [truncated by gateway]")

        print("Checking documentation is not added to system prompt...")
        assert doc == ""
...
class TestBuildKiroPayloadImages:
    ...
    def test_default_payload_omits_gateway_prompt_enrichments(self):
        """
        What it does: Verifies the default payload stays free of gateway prompt enrichments.
        Purpose: Ensure strict clients get a cleaner prompt surface by default.
        """
        print("Setup: Single user message with default prompt settings...")
        messages = [UnifiedMessage(role="user", content="Hello from Claude Code")]

        print("Action: Building Kiro payload with default prompt settings...")
        result = build_kiro_payload(
            messages=messages,
            system_prompt="",
            model_id="claude-sonnet-4",
        ...
        print("Checking content remains clean...")
        assert content == "Hello from Claude Code"
        assert "<thinking_mode>" not in content
        assert "# Extended Thinking Mode" not in content
        assert "# Output Truncation Handling" not in content
