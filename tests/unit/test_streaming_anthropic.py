...
class TestStreamKiroToAnthropic:
    ...
    @pytest.mark.asyncio
    async def test_streaming_applies_stop_sequence_for_text_only_requests(
        self, mock_response, mock_model_cache, mock_auth_manager
    ):
        """
        What it does: Stops a text-only stream at the earliest stop sequence.
        Goal: Restore Anthropic stop sequence behavior without touching tool flows.
        """
        print("Setup: Mock stream with stop sequence split across chunks...")

        async def mock_parse_kiro_stream(*args, **kwargs):
            yield KiroEvent(type="content", content="AAA ST")
            yield KiroEvent(type="content", content="OP BBB")
        ...
        assert text_joined == "AAA "
        assert '"stop_reason": "stop_sequence"' in message_delta_event
        assert '"stop_sequence": "STOP"' in message_delta_event
        print("✓ Streaming stop sequence applied locally")

    @pytest.mark.asyncio
    async def test_streaming_applies_max_tokens_for_text_only_requests(
        self, mock_response, mock_model_cache, mock_auth_manager
    ):
        """
        What it does: Stops a text-only stream when local max_tokens budget is reached.
        Goal: Restore Anthropic max_tokens behavior for safe text-only streaming.
        """
        print("Setup: Mock stream with text longer than token budget...")

        async def mock_parse_kiro_stream(*args, **kwargs):
            yield KiroEvent(type="content", content="01234")
            yield KiroEvent(type="content", content="56789")
            yield KiroEvent(type="content", content="ABCDE")

        def fake_count_tokens(text, apply_claude_correction=True):
            return len(text)
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "kiro.streaming_anthropic.count_tokens",
                    side_effect=fake_count_tokens,
                ):
                    async for event in stream_kiro_to_anthropic(
                        mock_response,
                        "claude-sonnet-4",
    ...
    async def test_streaming_text_controls_are_opt_in_and_do_not_touch_tool_flows(
        ...
        """
        What it does: Leaves tool-oriented streams unchanged when local controls are disabled.
        Goal: Ensure agentic tool behavior is not weakened by streaming compatibility fixes.
        """
        print("Setup: Mock tool-oriented stream with text and tool call...")
        ...
        async def mock_parse_kiro_stream(*args, **kwargs):
            yield KiroEvent(type="content", content="AAA STOP BBB")
            yield KiroEvent(type="tool_use", tool_use=tool_use_data)
        ...
        with patch(
            ...
            with patch(
                ...
                async for event in stream_kiro_to_anthropic(
                    ...
                    enable_local_text_controls=False,
                ):
                    events.append(event)

        delta_events = [event for event in events if "content_block_delta" in event]
        assert any("AAA STOP BBB" in event for event in delta_events)
        assert any("tool_use" in event for event in events)
        print("✓ Tool flow bypasses local streaming text controls")
...
class TestCollectAnthropicResponse:
    ...
    @pytest.mark.asyncio
    async def test_applies_stop_sequences_to_plain_text_response(
        self, mock_response, mock_model_cache, mock_auth_manager
    ):
        """
        What it does: Trims plain-text output at the earliest stop sequence.
        Goal: Restore Anthropic stop sequence semantics for non-streaming text replies.
        """
        print("Setup: Mock stream result with stop sequence inside text...")
        ...
        assert result["content"][0]["text"] == "AAA "
        assert result["stop_reason"] == "stop_sequence"
        assert result["stop_sequence"] == "BBB"
        assert result["_kiro_gateway_meta"]["local_stop_control"] == "stop_sequence"
        assert result["_kiro_gateway_meta"]["content_truncation"] == "none"
        print("✓ stop_sequences are applied locally")

    @pytest.mark.asyncio
    async def test_stop_sequence_wins_when_it_appears_before_max_tokens(
        self, mock_response, mock_model_cache, mock_auth_manager
    ):
        """
        What it does: Prefers stop_sequence when it appears before max_tokens cut-off.
        Goal: Match the earliest termination condition.
        """
        print("Setup: Mock stream result with both stop sequence and long text...")

        mock_result = StreamResult(
            content="alpha STOP omega tail",
            thinking_content="",
            tool_calls=[],
            usage=None,
        ...
        print("✓ Earliest stop condition wins")

    @pytest.mark.asyncio
    async def test_applies_max_tokens_to_plain_text_response(
        self, mock_response, mock_model_cache, mock_auth_manager
    ):
        """
        What it does: Truncates plain-text output to the requested max_tokens budget.
        Goal: Restore Anthropic max_tokens behavior without touching tool calls.
        """
        print("Setup: Mock stream result with text longer than token budget...")
        ...
        with patch(
            ...
            with patch(
                "kiro.streaming_anthropic.count_tokens", side_effect=fake_count_tokens
            ):
                result = await collect_anthropic_response(
                    mock_response,
                    "claude-sonnet-4",
        ...
        assert result["content"][0]["text"] == "0123456789"
        assert result["stop_reason"] == "max_tokens"
        assert result["stop_sequence"] is None
        assert result["usage"]["output_tokens"] == 10
        assert result["_kiro_gateway_meta"]["local_stop_control"] == "max_tokens"
        print("✓ max_tokens are applied locally")

    @pytest.mark.asyncio
    async def test_max_tokens_does_not_break_tool_use_response(
        self, mock_response, mock_model_cache, mock_auth_manager
    ):
        """
        What it does: Leaves tool_use responses untouched even when max_tokens is small.
        Goal: Avoid degrading agentic tool behavior.
        """
        print("Setup: Mock stream result with tool call and small max_tokens...")
        ...
        print("✓ Tool responses bypass local token truncation")
