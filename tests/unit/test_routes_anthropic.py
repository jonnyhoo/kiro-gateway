...
class TestAnthropicExactResponseCache:
    """Tests for Redis-backed exact response cache on Anthropic route."""

    @patch("kiro.routes_anthropic.KiroHttpClient")
    def test_non_streaming_returns_cached_response_without_upstream_call(
        self, mock_kiro_http_client_class, test_client, valid_proxy_api_key
    ):
        print("\n--- Test: Anthropic exact cache hit ---")
        ...
        test_client.app.state.response_cache = mock_cache
    ...
    @patch("kiro.routes_anthropic.KiroHttpClient")
    def test_non_streaming_cache_miss_fetches_and_stores_response(
        self,
        ...
        print("\n--- Test: Anthropic exact cache miss ---")
        ...
        assert response.headers["x-kiro-gateway-content-truncation"] == "none"
        assert response.headers["x-kiro-gateway-content-recovery"] == "none"
    ...
    @patch("kiro.routes_anthropic.collect_anthropic_response", new_callable=AsyncMock)
    @patch("kiro.routes_anthropic.KiroHttpClient")
    def test_non_streaming_auto_continues_truncated_text_response(
        self,
        mock_kiro_http_client_class,
        mock_collect_anthropic_response,
        test_client,
        ...
        mock_client_instance = AsyncMock()
        mock_client_instance.request_with_retry = AsyncMock(
            side_effect=[Mock(status_code=200), Mock(status_code=200)]
        )
        mock_client_instance.close = AsyncMock()
        mock_client_instance.client = AsyncMock()
        mock_kiro_http_client_class.return_value = mock_client_instance
        ...
        assert response.status_code == 200
        assert response.headers["x-kiro-gateway-content-truncation"] == "recovered"
        assert response.headers["x-kiro-gateway-content-recovery"] == "continued:1"
        assert response.headers["x-kiro-gateway-system-transport"] == "history_prelude"
        assert response.json()["content"][0]["text"] == "ABCDEFghi"
        assert "_kiro_gateway_meta" not in response.json()
        assert mock_client_instance.request_with_retry.await_count == 2

    @patch("kiro.routes_anthropic.collect_anthropic_response", new_callable=AsyncMock)
    @patch("kiro.routes_anthropic.KiroHttpClient")
    def test_non_streaming_auto_continues_incomplete_text_without_truncation_flag(
        self,
        mock_kiro_http_client_class,
        mock_collect_anthropic_response,
        test_client,
        ...
        mock_collect_anthropic_response.side_effect = [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": 'export const CONST_0171 = "abcdefghijklmnopqrstuvwxyz',
                    }
                ],
                "model": "claude-sonnet-4-5",
                "stop_reason": "end_turn",
    ...
    @patch("kiro.routes_anthropic.collect_anthropic_response", new_callable=AsyncMock)
    @patch("kiro.routes_anthropic.KiroHttpClient")
    def test_non_streaming_tool_result_request_reports_tool_cache_status(
        self,
        mock_kiro_http_client_class,
        mock_collect_anthropic_response,
        test_client,
        ...
        print("\n--- Test: Anthropic tool result cache header ---")

        response_cache = MagicMock()
        response_cache.is_available.return_value = True
        response_cache.get_json = AsyncMock(return_value=None)
        response_cache.set_json = AsyncMock(return_value=True)
        test_client.app.state.response_cache = response_cache
    ...
    @patch("kiro.routes_anthropic.collect_anthropic_response", new_callable=AsyncMock)
    @patch("kiro.routes_anthropic.KiroHttpClient")
    def test_non_streaming_reports_prompt_cache_status_and_usage(
        self,
        mock_kiro_http_client_class,
        mock_collect_anthropic_response,
        test_client,
        ...
        print("\n--- Test: Anthropic prompt cache compatibility header/usage ---")
        ...
        prompt_cache = MagicMock()
        prompt_cache.evaluate_anthropic_request = AsyncMock(
            return_value=(
                {
        ...
        assert response.headers["x-kiro-gateway-prompt-cache-source"] == "legacy"
        assert response.headers["x-kiro-gateway-prompt-cache-segments"] == "unavailable"
        assert (
            response.headers["x-kiro-gateway-prompt-cache-tokens"]
            == "read=22;create=11;5m=11;1h=0"
        )
        assert (
            response.headers["x-kiro-gateway-prompt-cache-volatility"] == "unavailable"
        )
        assert (
            response.headers["x-kiro-gateway-prompt-cache-normalized"] == "unavailable"
        )
        body = response.json()
        assert body["usage"]["cache_creation_input_tokens"] == 11
        assert body["usage"]["cache_read_input_tokens"] == 22
        assert body["usage"]["cache_creation"]["ephemeral_5m_input_tokens"] == 11

    @patch("kiro.routes_anthropic.KiroHttpClient")
    def test_streaming_reports_prompt_cache_debug_headers(
        self, mock_kiro_http_client_class, test_client, valid_proxy_api_key
    ):
        print("\n--- Test: Anthropic streaming prompt cache debug headers ---")
            ...
            return_value=PromptCacheEvaluation(
                ...
                segment_results=[
                    ...
                    CacheSegmentResult(
                        ...
                        token_count=6,
                        ttl_seconds=3600,
                        volatility_tags=["volatile_timestamp"],
                        normalization_tags=["normalized_timestamp"],
                    ),
                ],
                source="explicit",
            )
        ...
        assert response.headers["x-kiro-gateway-prompt-cache-tokens"] == (
            "read=10;create=6;5m=0;1h=6"
        )
        assert response.headers["x-kiro-gateway-prompt-cache-volatility"] == (
            "system[0]:volatile_timestamp"
        )
    ...
    def test_streaming_text_only_requests_forward_local_output_controls(
        ...
        assert callable(captured_kwargs["auto_continue_callback"])
        assert captured_kwargs["max_auto_continuation_rounds"] == 2
        assert response.headers["x-kiro-gateway-local-text-controls"] == "enabled"

    @patch("kiro.routes_anthropic.collect_anthropic_response", new_callable=AsyncMock)
    @patch("kiro.routes_anthropic.KiroHttpClient")
    def test_non_streaming_shadow_prompt_cache_source_without_explicit_cache_control(
        self,
        mock_kiro_http_client_class,
        mock_collect_anthropic_response,
        test_client,
        ...
        print("\n--- Test: Anthropic shadow prompt cache source header ---")
            ...
            return_value=PromptCacheEvaluation(
                ...
                segment_results=[
                    CacheSegmentResult(
                        ...
                        token_count=10,
                        ttl_seconds=3600,
                        volatility_tags=["volatile_request_id"],
                        normalization_tags=["normalized_request_id"],
                    ),
                    CacheSegmentResult(
                        namespace="message",
                        label="message[0:0]",
        ...
        response = test_client.post(
            ...
            json={
                "model": "claude-sonnet-4-5",
                "max_tokens": 100,
                "system": "request_id=req_a1 stable system",
                "messages": [{"role": "user", "content": "Hello"}],
                "stream": False,
            },
        )
        ...
        assert response.status_code == 200
        assert response.headers["x-kiro-gateway-prompt-cache-source"] == "shadow"
        assert response.headers["x-kiro-gateway-prompt-cache-volatility"] == (
            "system[0]:volatile_request_id"
        )
        assert response.headers["x-kiro-gateway-prompt-cache-normalized"] == (
            "system[0]:normalized_request_id"
        )
