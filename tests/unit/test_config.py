...
class TestPromptFeatureDefaults:
    """Tests for prompt-related default configuration values."""

    def test_fake_reasoning_default_is_disabled(self):
        """
        What it does: Verifies the default value for FAKE_REASONING.
        Purpose: Ensure prompt pollution is disabled by default.
        """
        print("Setup: Removing FAKE_REASONING from environment...")

        with patch.dict(os.environ, {}, clear=False):
            if "FAKE_REASONING" in os.environ:
                del os.environ["FAKE_REASONING"]
            ...
            print(f"FAKE_REASONING_ENABLED: {config_module.FAKE_REASONING_ENABLED}")
            assert config_module.FAKE_REASONING_ENABLED is False

    def test_truncation_recovery_default_is_disabled(self):
        """
        What it does: Verifies the default value for TRUNCATION_RECOVERY.
        Purpose: Ensure synthetic prompt injections are disabled by default.
        """
        print("Setup: Removing TRUNCATION_RECOVERY from environment...")

        with patch.dict(os.environ, {}, clear=False):
            if "TRUNCATION_RECOVERY" in os.environ:
                del os.environ["TRUNCATION_RECOVERY"]
            ...
            print(f"TRUNCATION_RECOVERY: {config_module.TRUNCATION_RECOVERY}")
            assert config_module.TRUNCATION_RECOVERY is False
