...
class TestStreamResult:
    ...
    def test_creates_empty_result(self):
        ...
        assert result.content_was_truncated is False
...
class TestCollectStreamToResult:
    ...
    async def test_collects_context_usage_percentage(self, mock_response, mock_parser):
        ...
        assert result.stream_completed_normally is True
        assert result.content_was_truncated is False
    ...
    @pytest.mark.asyncio
    async def test_marks_plain_text_without_completion_signal_as_truncated(
        self, mock_response
    ):
        """
        What it does: Marks plain text as truncated when completion signals are missing.
        Goal: Surface silent upstream cut-offs to the caller.
        """
        print("Setup: Mock parse result without context usage...")

        mock_events = [KiroEvent(type="content", content="partial text only")]
        ...
        assert result.stream_completed_normally is False
        assert result.content_was_truncated is True
        print("✓ Missing completion signal is marked as truncation")

    @pytest.mark.asyncio
    async def test_tool_calls_do_not_trigger_plain_text_truncation_flag(
        self, mock_response
    ):
        """
        What it does: Avoids marking tool responses as plain-text truncation.
        Goal: Keep tool truncation handling separate from text truncation handling.
        """
        print("Setup: Mock parse result with tool call and no context usage...")

        mock_events = [
            KiroEvent(type="content", content="planning"),
            KiroEvent(
                type="tool_use",
                tool_use={
                    "id": "call_1",
        ...
        assert result.stream_completed_normally is False
        assert result.content_was_truncated is False
        print("✓ Tool flow is not misclassified as plain-text truncation")
