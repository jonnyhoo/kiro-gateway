...
class TestBuildKiroPayloadToolCallsIntegration:
    ...
    def test_long_tool_description_truncated_in_payload(self):
        """
        What it does: Verifies integration of long tool descriptions into payload.
        Purpose: Ensure long descriptions are truncated instead of copied into the prompt.
        """
        ...
        print("Checking that system prompt remains clean...")
        current_content = result["conversationState"]["currentMessage"][
            "userInputMessage"
        ]["content"]
        assert "You are helpful" in current_content
        assert "## Tool: long_tool" not in current_content
        assert long_desc not in current_content

        print("Checking that tool description is truncated in context...")
        tools_context = result["conversationState"]["currentMessage"][
            "userInputMessage"
        ]["userInputMessageContext"]["tools"]
        description = tools_context[0]["toolSpecification"]["description"]
        assert description.endswith("... [truncated by gateway]")
