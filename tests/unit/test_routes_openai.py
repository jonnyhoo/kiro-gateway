...
class TestOpenAIExactResponseCache:
    """Tests for Redis-backed exact response cache on OpenAI route."""

    @patch("kiro.routes_openai.KiroHttpClient")
    def test_non_streaming_returns_cached_response_without_upstream_call(
        self, mock_kiro_http_client_class, test_client, valid_proxy_api_key
    ):
        print("\n--- Test: OpenAI exact cache hit ---")
        ...
        assert response.json()["id"] == "chatcmpl_cached"
    ...
    def test_non_streaming_cache_miss_fetches_and_stores_response(
        ...
        mock_collect_stream_response,
        test_client,
        valid_proxy_api_key,
    ):
        print("\n--- Test: OpenAI exact cache miss ---")
        ...
        mock_collect_stream_response.return_value = {
            "id": "chatcmpl_live",
            "object": "chat.completion",
            "model": "claude-sonnet-4-5",
            "choices": [],
        ...
        assert response.headers["x-kiro-gateway-tool-cache"] == "bypass"
        assert response.json()["id"] == "chatcmpl_live"
        assert mock_kiro_http_client_class.called
    ...
    def test_non_streaming_tool_result_request_reports_tool_cache_status(
        ...
        print("\n--- Test: OpenAI tool result cache header ---")
