...
class ToolCallTruncationError(Exception):
    """Raised when upstream truncates tool-call arguments."""
    ...
    def __init__(
        ...
        self.tool_call_id = tool_call_id or "unknown"
        self.truncation_info = truncation_info or {}
        size_bytes = self.truncation_info.get("size_bytes", "unknown size")
        reason = self.truncation_info.get("reason", "unknown reason")
        super().__init__(
            f"Tool call truncated by Kiro API: tool='{self.tool_name}', "
            f"id={self.tool_call_id}, size={size_bytes}, reason={reason}"
        )
...
async def parse_kiro_stream(
    ...
    try:
        ...
        for tc in all_tool_calls:
            if tc.get("_truncation_detected") and not TRUNCATION_RECOVERY:
                func = tc.get("function") or {}
                raise ToolCallTruncationError(
