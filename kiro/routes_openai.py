...
from kiro.response_cache import get_openai_cache_eligibility
    ...
    if not request_data.stream:
        if cacheable and response_cache.is_available():
            cached_response = await response_cache.get_json(
                "openai-chat-completions", cache_payload
            )
            if cached_response is not None:
                logger.info(
                    "HTTP 200 - POST /v1/chat/completions (non-streaming) - cache hit"
                )
                if debug_logger:
            ...
            logger.debug(
                f"Response cache bypass for /v1/chat/completions: {cache_reason}"
            )
    ...
    try:
        ...
        if request_data.stream:
            ...
            async def stream_wrapper():
                ...
                try:
                    ...
                    if streaming_error:
                        ...
                        logger.info(
                            "HTTP 200 - POST /v1/chat/completions (streaming) - completed"
                        )
            ...
            return StreamingResponse(
                ...
                headers={"x-kiro-gateway-tool-cache": tool_cache_status},
            )
            ...
            logger.info(
                "HTTP 200 - POST /v1/chat/completions (non-streaming) - completed"
            )
            ...
            if cache_status == "miss":
                await response_cache.set_json(
                    "openai-chat-completions", cache_payload, openai_response
                )
