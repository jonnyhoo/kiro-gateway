...
def process_tools_with_long_descriptions(
    ...
    If description exceeds the limit, it is truncated in-place to avoid
    polluting the prompt with duplicated tool documentation.
    ...
    Returns:
        ...
        - String with documentation to add to system prompt (always empty)
    """
    if not tools:
        return None, ""
    ...
    for tool in tools:
        ...
        if len(description) <= TOOL_DESCRIPTION_MAX_LENGTH:
            ...
            logger.debug(
                ...
                "truncating description to keep prompt clean"
            )

            suffix = "... [truncated by gateway]"
            if TOOL_DESCRIPTION_MAX_LENGTH <= len(suffix):
                truncated_description = description[:TOOL_DESCRIPTION_MAX_LENGTH]
            else:
                head = description[: TOOL_DESCRIPTION_MAX_LENGTH - len(suffix)].rstrip()
                truncated_description = head + suffix
    ...
    return processed_tools if processed_tools else None, ""
